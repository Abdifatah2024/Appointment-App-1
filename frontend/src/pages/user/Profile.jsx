...
import React, { useEffect, useMemo, useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { Save, User2, Shield, Mail, Camera, X, KeyRound } from "lucide-react";
import {
  updateMyProfile,
  uploadMyAvatar,
...
export default function Profile() {
  ...
  const {
    ...
    // ✅ NEW password states (assumes you will add to authSlice)
    passwordSaving,
    passwordError,
    passwordSuccess,
  } = useSelector((s) => s.auth);
  ...
  useEffect(() => {
    if (profileError || profileSuccess || avatarError || passwordError || passwordSuccess) {
      const t = setTimeout(() => dispatch(clearProfileMessage()), 3000);
      return () => clearTimeout(t);
    }
  }, [profileError, profileSuccess, avatarError, passwordError, passwordSuccess, dispatch]);
  ...
  const onSave = async () => {
    ...
    await dispatch(updateMyProfile({ fullName: nextName, bio: nextBio }));
  };
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div>
        <h1 className="text-xl font-black text-slate-900">Profile</h1>
        <p className="text-sm text-slate-500 mt-1">
          Manage your account information (edit name, bio, password & profile photo).
        </p>
      </div>
          ...
          <div className="mt-6 pt-6 border-t border-slate-100">
            ...
            {user?.provider === "google" ? (
              <p className="mt-3 text-sm text-slate-500 font-semibold">
                This account uses Google login. Password update is disabled.
              </p>
            ) : (
              <div className="mt-4 space-y-3">
                ...
                <button
                  ...
                  className="w-full mt-2 inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2 bg-slate-900 text-white font-black hover:bg-slate-800 disabled:opacity-60"
                >
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl p-6">
          ...
          <div className="mt-5">
            ...
            <textarea
              ...
              rows={4}
              maxLength={250}
              className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-200 resize-none"
              placeholder="Write a short bio (max 250 characters)..."
            />
            <div className="mt-2 text-xs text-slate-500 font-semibold flex justify-between">
              <span>Professional summary / short description</span>
